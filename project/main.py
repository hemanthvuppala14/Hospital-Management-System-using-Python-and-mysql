...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/pt-2'
db=SQLAlchemy(app)
...
def signup():
    if request.method=="POST":
        ...
        db.session.commit()
        mail.send_message(
                subject='Hospital Data management system',
                sender='hemanthvuppala14@gmail.com',
                recipients=[email],
                body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nsrfid: {srfid}\n\ndob {dob}\n\n confirmation mail for signup \n\n\nThank You..."
)
...
def hospitalUser():
    ...
    if('user' in session and session['user']=="admin"):
        ...
        if request.method=="POST":
           ...
           # mail.send_message('Hospital Data management system',sender=Param('hemanthvuppala14@gmail.com'),recipients=(email),body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You..." )
            mail.send_message(
                subject='Hospital Data management system',
                sender='hemanthvuppala14@gmail.com',
                recipients=[email],
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        if bedtype == "NormalBed":
            seat = check2.normalbed
            check2.normalbed = seat - 1
        elif bedtype == "HICUBed":
            seat = check2.hicubed
            check2.hicubed = seat - 1
        elif bedtype == "ICUBed":
            seat = check2.icubed
            check2.icubed = seat - 1
